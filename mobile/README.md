...
- Node.js 22 или новее в поддерживаемом диапазоне проекта (`>=22 <25` для
  сайта/API; `.nvmrc` находится в корне репозитория);
- npm;
...
Если на машине установлен `nvm`, перед `npm ci` можно выполнить `nvm use`, чтобы
выбрать версию из `.nvmrc`. Если команда `nvm` недоступна, это не блокер:
достаточно использовать установленный Node.js из поддерживаемого диапазона.
...
Если у вас установлен `nvm`, вместо проверки версии можно выполнить `nvm use`.
Если терминал отвечает `zsh: command not found: nvm`, просто пропустите этот
шаг или установите Node.js 22 другим способом, например через Homebrew.
...
Если в приложении отображается «Не удалось загрузить дефекты», сначала
убедитесь, что этот терминал всё ещё открыт и в нём есть строки
`Local dev server ready: http://localhost:8888` и `Loaded function issues`.
Затем выполните проверку API выше: для iOS Simulator должен отвечать именно
`http://127.0.0.1:8888/api/issues`.
